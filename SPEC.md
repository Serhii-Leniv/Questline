...
- **Bring-your-own provider / локальна модель (✅ реалізовано для OpenAI-сумісних).** Архітектура ізолює AI за власним інтерфейсом `LlmClient` (пакет `ai/`), тож провайдер перемикається **лише конфігом**, без зміни доменного коду:
  ...
  - **[future] Власне НЕ-OpenAI-сумісне API:** друга імплементація `LlmClient` (свій `RestClient`) — сейм уже готовий, лишається написати клас.
  - **Принцип:** сервісний шар залежить лише від `LlmClient` + наших record-ів; уся специфіка провайдера — у конфізі та модулі `ai/`. Жодних провайдер-ключів на клієнті.
