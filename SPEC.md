...
- **Bring-your-own provider / локальна модель (✅ реалізовано для OpenAI-сумісних).** Архітектура ізолює AI за власним інтерфейсом `LlmClient` (пакет `ai/`), тож провайдер перемикається **лише конфігом**, без зміни доменного коду:
  ...
  - **Per-user BYOK (✅ реалізовано).** Кожен користувач може вписати **свій** OpenAI-сумісний ендпоінт + модель + ключ у Settings (`PUT /api/me/ai-settings`). Ключ шифрується at rest (AES-GCM, `SecretCipher`). Коли заданий — AI-джоби цього юзера йдуть прямим `RestClient`-викликом до його провайдера (`ai/OpenAiCompatibleClient`, без Spring AI), з тим самим repair-loop; інакше — серверний дефолт. Ключ ніколи не повертається клієнту.
