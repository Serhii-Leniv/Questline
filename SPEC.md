...
- **Multi-tenancy:** кожна доменна сутність має `userId`; **усі** запити скоупляться по сесійному юзеру (через Spring Security `Authentication` + сервісний шар). Жодного запиту без власника.
  - **Postgres RLS (✅ реалізовано, другий рубіж).** `RlsTransactionManager` виставляє GUC `app.user_id` з `SecurityContext` на старті кожної транзакції (`SET LOCAL`, скидається на commit). Політики (`V10`) ізолюють user-таблиці по `app.user_id`, але **пропускають доступ коли GUC не виставлено** (`nullif(...,'')` → NULL) — щоб фонові джоби/startup працювали. `FORCE ROW LEVEL SECURITY` вмикає політику й для власника. ⚠️ У проді запускати застосунок **не як superuser/owner** (superuser завжди обходить RLS). Тест ізоляції — через окрему обмежену роль (`RlsIsolationIntegrationTest`).
...
- **Bring-your-own provider / локальна модель (✅ реалізовано для OpenAI-сумісних).** Архітектура ізолює AI за власним інтерфейсом `LlmClient` (пакет `ai/`), тож провайдер перемикається **лише конфігом**, без зміни доменного коду:
  - **Перемикач:** `AI_PROVIDER=google-genai` (дефолт, Gemini) або `openai`. Для `openai` — `OPENAI_BASE_URL` + `OPENAI_API_KEY` + `OPENAI_MODEL`.
  - **Локальні / self-hosted:** Ollama (`http://localhost:11434`), LM Studio (`http://localhost:1234`), vLLM — усі через OpenAI-сумісний ендпоінт. Будь-який ключ для локальних серверів.
  - **Хмарні OpenAI-сумісні:** Groq (`https://api.groq.com/openai`), OpenRouter, OpenAI.
  - **Per-user BYOK (✅ реалізовано).** Кожен користувач може вписати **свій** OpenAI-сумісний ендпоінт + модель + ключ у Settings (`PUT /api/me/ai-settings`). Ключ шифрується at rest (AES-GCM, `FieldCipher`). Коли заданий — AI-джоби цього юзера йдуть прямим `RestClient`-викликом до його провайдера (`ai/OpenAiCompatibleClient`, без Spring AI), з тим самим repair-loop; інакше — серверний дефолт. Ключ ніколи не повертається клієнту.
  - **Принцип:** сервісний шар залежить лише від `LlmClient` + наших record-ів; уся специфіка провайдера — у конфізі/per-user-налаштуваннях та модулі `ai/`. Жодних провайдер-ключів на клієнті.
