...
- **[future] Bring-your-own provider / локальна модель.** Архітектура вже ізолює AI за власним інтерфейсом `LlmClient` (пакет `ai/`), тож на майбутнє користувач має змогу підключити **будь-який власний ендпоінт або локальну модель** без зміни доменного коду:
  - **Локальні / self-hosted:** Ollama, LM Studio, llama.cpp, vLLM тощо — через OpenAI-сумісний `base-url` (напр. `http://localhost:11434/v1`) або окремий Spring AI Ollama-старлет.
  - **Власне API:** будь-який кастомний HTTP-ендпоінт — друга імплементація `LlmClient` (свій `RestClient`), вибір через профіль/конфіг (`app.ai.provider=gemini|openai|ollama|custom`).
  - **Принцип:** доменний/сервісний шар залежить лише від `LlmClient` + наших record-ів; уся специфіка провайдера (ключі, `base-url`, модель, формат) — у конфізі та в одному модулі `ai/`. Жодних провайдер-ключів на клієнті.
