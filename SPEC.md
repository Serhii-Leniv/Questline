...
- **Multi-tenancy:** кожна доменна сутність має `userId`; **усі** запити скоупляться по сесійному юзеру (через Spring Security `Authentication` + сервісний шар). Жодного запиту без власника. (Phase 3: Postgres RLS як другий рубіж — **зробити уважно**: фонові джоби ходять у БД без user-контексту, тож політики мають пропускати «service»-доступ (GUC unset) або джоби мають виставляти `app.user_id`; застосунок має конектитись **не як superuser/owner** (інакше RLS обходиться), а тести RLS — через окрему обмежену роль.)
